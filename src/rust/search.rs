...
pub fn load_data() -> MergedUltraSlim {
    ...
    let file_contents = fs::read_to_string(source_dir.to_str().unwrap().to_owned() + "/dist/" + FILE_NAME)
...
mod tests {
    ...
    fn test_load_data() {
        ...
        assert!(data.get_by_name("adadadadad", SearchType::MariaDB).is_err());
        assert!(data.get_by_name("adadadadad", SearchType::MySQL).is_err());
        assert!(data.get_by_name("adadadadad", SearchType::Any).is_err());
        ...
        assert!(data.get_by_name("ndb_log_orig", SearchType::MariaDB).is_err());
